...
cd "{CWD_PATH}" && codex exec -p analyst -a never -o /tmp/codex-response.md - < /tmp/second-opinion-prompt.md

# If no --cwd (vault root, not a git repo):
cd "/Users/ishtyler/Documents/My Brain 2.0" && codex exec -p analyst -a never --skip-git-repo-check -o /tmp/codex-response.md - < /tmp/second-opinion-prompt.md
```
...
- If Codex times out → Report timeout, suggest simpler query or --model flag
...
**Engine**: OpenAI Codex CLI v0.114.0 (multi-agent orchestration)
