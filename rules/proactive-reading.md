...
- Project-specific commands available (e.g., K-Town has `/ktown-plan`, `/ktown-implement`) → suggest those first
  - `ktown-test` skill (`.claude/skills/ktown-test/`) — K-Town-specific testing operational modes (8 modes including inventory, feature-test design, targeted regression, gameplay simulation, visual+perf gates, flake triage, fixture maintenance, gotcha capture). Defers to `regression-testing-protocol.md` for global discipline; forward-references FP21 (Master Test-Fixture Layer) for typed-harness/Colyseus-fixture/multiplayer-specs/perf-budgets/visual-policy infrastructure. Use when planning or implementing any K-Town test work.
- `.claude/rules/ci-gate-discipline.md` — MANDATORY when modifying CI workflows or responding to CI failures. Codifies the response hierarchy (stash-revert → register → fix → only-then-disable-with-criteria), forbidden patterns, atomic fix-and-enable rule, and CI-disable-audit enforcement. Reinforces ktown-test mode 6 (flake-and-failure-triage).
