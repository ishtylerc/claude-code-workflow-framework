...
- `/second-opinion <topic> [--files=path1,path2,...] [--cwd=project-path] [--research] [--diagnose]` - Get a structured second opinion from OpenAI Codex CLI with multi-agent analysis. Claude forms independent position first, then Codex provides blind analysis. Produces structured comparison document in `thoughts/second-opinions/`. Requires Codex CLI + OPENAI_API_KEY. **Flags**: `--research` runs Claude-side internal+external research (research-specialist agents, parallel, anti-overlap enforced) and produces a structured Research Brief embedded in Codex's prompt with anti-anchoring framing (~3-5 min add-on). `--diagnose` runs heavy env checks (npm/network/config/cache/model probes) and exits — pair with `--quick` to skip slow probes. The two flags are mutually exclusive. Every non-`--diagnose` invocation prints a local-only one-line banner with CLI version, model, sandbox, and agent settings. The command autonomously classifies topic shape (ideation / architecture / tooling / bug / strategy / code-review) with deliberate ideation-default bias when in doubt — when classified as ideation, Codex's prompt gains an "Ideation Mode" lens shared across all 4 agents (surface holes, probe scope, find integration points, invite domain-expert pushback, resist premature convergence).
